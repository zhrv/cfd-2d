...
void FVM_TVD_IMPLICIT::run() 
{
	...
	if (STEADY)		log((char*)"Steady-State Flow\n");
	else			log((char*)"Unsteady-State Flow\n");
	log((char*)"TMAX = %e STEP_MAX = %d\n", TMAX, STEP_MAX);
	log((char*)"Flux calculation method: %s\n", FLUX_NAMES[FLUX]);
	
	// инициализируем портрет матрицы
	log((char*)"Matrix structure initialization:\n");
		...
		if (solverErr == MatrixSolver::RESULT_OK) {
			...
			if (step % PRINT_STEP == 0)
			{
				...
				if (!STEADY) {
					...
					log((char*)"step: %d\ttime step: %.16f\tmax iter: %d\tlim: %d\ttime: %d ms\n", step, t, maxIter, limCells, timeEnd-timeStart);
				}
				else {
					log((char*)"step: %d\tmax iter: %d\tlim: %d\ttime: %d ms\n", step, maxIter, limCells, timeEnd - timeStart);
				}
			}
...
void FVM_TVD_IMPLICIT::decCFL() 
{
	if (CFL > 0.1) {
		CFL *= 0.75;
		if (CFL < 0.1) CFL = 0.1;
		log((char*)" CFL Number has been decreased : %f \n", CFL);
	}
}
...
void FVM_TVD_IMPLICIT::incCFL() 
{
	if (CFL < maxCFL) {
		...
		log((char*)" CFL Number has been increased : %f \n", CFL);
	}
}
