	...
	while (t < TMAX)
	{
		...
		for (int iCell = 0; iCell < nc; ++iCell)
		{
			...
			for (int neighborIndex = 0; neighborIndex < 3; ++neighborIndex)
			{
				...
				if (neighbor >= 0)
				{
					// вычисление коэффециента. результат вычисления будет записан в tempMtx4.
			
					//
					//mtx.setMatrElement(iCell, neighbor, tempMtx4); // добавляем в таблицу значение в соседних ячейках.
				} else 
				{
					//
