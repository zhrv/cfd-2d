	...
	while (t < TMAX) 
	{
		...
		if (step % PRINT_STEP == 0)
		{
			if (!STEADY) {
				...
				log((char*)"step: %6d\tres: ro:%10.5e ru:%10.5e rv:%10.5e re:%10.5e\n", step, ro_res, ru_res, rv_res, re_res);
			}
		}
...
Region & FEM_RKDG::getRegionByName(char* name)
{
	...
	log((char*)"ERROR: unknown cell name '%s'...\n", name);
	EXIT(1);
}
...
VECTOR FEM_RKDG::getF(int iCell, Point pt)
{
	...
	else if (FUNC_COUNT == 6)
	{
		...
		log((char*)"ERROR: wrong basic functions count!\n");
		EXIT(1);
	}
}
